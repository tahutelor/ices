        ...
        switch($method){
            case 'product':
                ...
                foreach($t_data as $idx=>$row){
                    $row->product_status = SI::get_status_attr(
                        SI::type_get('Product_Engine',$row->product_status,'$status_list')['text']
                    );
                    $row->product_category_text = Tools::html_tag('strong',$row->product_category_code)
