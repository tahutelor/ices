<script>
    var customer_mail_address_methods = {
        load_mail_address: function (iparam) {
            ...
            customer_tbl_mail_address_method.reset();
            customer_tbl_mail_address_method.head_generate();

            $.each(iparam.mail_address, function (lidx, lrow) {
                customer_tbl_mail_address_method.input_row_generate(lrow);
            });
            customer_tbl_mail_address_method.input_row_generate({});

        }
    };

    var customer_mail_address_bind_event = function () {
        var lparent_pane = customer_parent_pane;
        var lprefix_id = customer_component_prefix_id;

        customer_tbl_mail_address_method.setting.func_new_row_validation = function (iopt) {
        ...
        customer_tbl_mail_address_method.setting.func_get_data_table = function () {
            var lparent_pane = customer_parent_pane;
            var lprefix_id = customer_component_prefix_id;
//            var lresult = [];
        ...
        customer_tbl_mail_address_method.setting.func_row_bind_event = function (iopt) {
            var lparent_pane = customer_parent_pane;
            var lprefix_id = customer_component_prefix_id;
            var lrow = iopt.tr;
...
<?php // --- End Of Show and Hide phase ---                  ?>
        ...
        customer_tbl_mail_address_method.setting.func_row_transform_comp_on_new_row = function (iopt) {
            var lrow = iopt.tr;
        ...
        customer_tbl_mail_address_method.setting.func_row_data_assign = function (iopt) {
            var lparent_pane = customer_parent_pane;
            var lprefix_id = customer_component_prefix_id;
            var ldata_row = iopt.data_row;
            var lrow = iopt.tr;
            ...
            switch (lmethod) {
                ...
                case 'view':
                    if (Object.keys(ldata_row).length > 0) {
                        ...
                        customer_tbl_mail_address_method.components.trash_set(iopt);
                    }
                    break;
