<script>
    var warehouse_address_methods = {
        load_address: function(iparam) {
            ...
            warehouse_tbl_address_method.reset();
            warehouse_tbl_address_method.head_generate();

            $.each(iparam.address, function(lidx, lrow) {
                warehouse_tbl_address_method.input_row_generate(lrow);
            });
            warehouse_tbl_address_method.input_row_generate({});

        }
    };

    var warehouse_address_bind_event = function() {
        var lparent_pane = warehouse_parent_pane;
        var lprefix_id = warehouse_component_prefix_id;

        warehouse_tbl_address_method.setting.func_new_row_validation = function(iopt) {
        ...
        warehouse_tbl_address_method.setting.func_get_data_table = function() {
            var lparent_pane = warehouse_parent_pane;
            var lprefix_id = warehouse_component_prefix_id;
            var lresult = {address: []};
        ...
        warehouse_tbl_address_method.setting.func_row_bind_event = function(iopt) {
            var lparent_pane = warehouse_parent_pane;
            var lprefix_id = warehouse_component_prefix_id;
            var lrow = iopt.tr;
...
<?php // --- End Of Show and Hide phase ---             ?>
        ...
        warehouse_tbl_address_method.setting.func_row_transform_comp_on_new_row = function(iopt) {
            var lrow = iopt.tr;
        ...
        warehouse_tbl_address_method.setting.func_row_data_assign = function(iopt) {
            var lparent_pane = warehouse_parent_pane;
            var lprefix_id = warehouse_component_prefix_id;
            var ldata_row = iopt.data_row;
            var lrow = iopt.tr;
            ...
            switch (lmethod) {
                ...
                case 'view':
                    if (Object.keys(ldata_row).length > 0) {
                        ...
                        warehouse_tbl_address_method.components.trash_set(iopt);
                    }
                    break;
            }
