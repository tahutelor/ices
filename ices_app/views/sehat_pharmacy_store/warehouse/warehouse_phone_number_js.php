<script>
    var warehouse_phone_number_methods = {
        load_phone_number: function (iparam) {
            ...
            warehouse_tbl_phone_number_method.reset();
            warehouse_tbl_phone_number_method.head_generate();

            $.each(iparam.phone_number, function (lidx, lrow) {
                warehouse_tbl_phone_number_method.input_row_generate(lrow);
            });
            warehouse_tbl_phone_number_method.input_row_generate({});

        }
    }

    var warehouse_phone_number_bind_event = function () {
        var lparent_pane = warehouse_parent_pane;
        var lprefix_id = warehouse_component_prefix_id;
        ...
        warehouse_tbl_phone_number_method.setting.func_new_row_validation = function (iopt) {
...
<?php //validation replace space or empty sting         ?>
        ...
        warehouse_tbl_phone_number_method.setting.func_get_data_table = function () {
            var lparent_pane = warehouse_parent_pane;
            var lprefix_id = warehouse_component_prefix_id;
//            var lresult = [];
        ...
        warehouse_tbl_phone_number_method.setting.func_row_bind_event = function (iopt) {
            var lparent_pane = warehouse_parent_pane;
            var lprefix_id = warehouse_component_prefix_id;
            var lrow = iopt.tr;
...
<?php // --- End Of Show and Hide phase ---                 ?>
        ...
        warehouse_tbl_phone_number_method.setting.func_row_transform_comp_on_new_row = function (iopt) {
            var lrow = iopt.tr;
        ...
        warehouse_tbl_phone_number_method.setting.func_row_data_assign = function (iopt) {
            var lparent_pane = warehouse_parent_pane;
            var lprefix_id = warehouse_component_prefix_id;
            var ldata_row = iopt.data_row;
            var lrow = iopt.tr;
            ...
            switch (lmethod) {
                ...
                case 'view':
                    if (Object.keys(ldata_row).length > 0) {
                        ...
                        $(lrow).find('[col_name="phone_number_type"]')[0].innerHTML = '<div>' + ldata_row.phone_number_type_text + '</div>';
                        warehouse_tbl_phone_number_method.components.trash_set(iopt);
                    }
                    break;
