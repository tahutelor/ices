    ...
    var rpt_product_bind_event = function(){
        ...
        $(lparent_pane).find(lprefix_id+'_module_name').on('change',function(){
            ...
            if($(this).select2('val')!== ''){
                ...
                var lajax_url = rpt_product_form_render_url+lmodule_name+'/';
        ...
        $(lparent_pane).find(lprefix_id+'_save_excel').on('click',function(){
            ...
            if(lmodule_name !== ''){
                var ljson_data = {};
                ljson_data = eval('rpt_product_'+lmodule_name+'_param_get()');
                window.open(rpt_product_index_url+'download_excel/'+lmodule_name+'/'+encodeURIComponent(JSON.stringify(ljson_data)));
            }
        });
        ...
        $(lparent_pane).find(lprefix_id+'_btn_preview').on('click',function(e){
            ...
            if(lmodule_name !== ''){
                ...
                ljson_data = eval('rpt_product_'+lmodule_name+'_param_get()');
                var lmodule_name = $(lparent_pane).find(lprefix_id+'_module_name').select2('val');
                var lajax_url = rpt_product_index_url+'rpt_preview/'+lmodule_name+'/';
