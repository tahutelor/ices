...
class Customer_Type_Renderer {
    ...
    public static function modal_customer_type_render($app, $modal) {
        //<editor-fold defaultstate="collapsed">
        $modal->header_set(array('title' => Lang::get('Product Category'), 'icon' => 'fa fa-cogs'));
        $modal->width_set('95%');
        $components = self::customer_type_components_render($app, $modal, true);
        //</editor-fold>
    }

    public static function customer_type_render($app, $form, $data, $path, $method) {
        //<editor-fold defaultstate="collapsed">
        $path = Customer_Type_Engine::path_get();

        $id_prefix = Customer_Type_Engine::$prefix_id;

        $id = $data['id'];
        $components = self::customer_type_components_render($app, $form, false);
    ...
    public static function customer_type_components_render($app, $form, $is_modal) {
        //<editor-fold defaultstate="collapsed">
        $path = Customer_Type_Engine::path_get();
        $components = array();
        $db = new DB();
        ...
        $form->input_select_add()
                ->input_select_set('label', 'Notification - Sales Invoice Outstanding')
                ->input_select_set('icon', 'fa fa-info')
                ->input_select_set('min_length', '0')
                ->input_select_set('id', $id_prefix . '_notif_si_outstanding')
        ...
        $form->input_select_add()
                ->input_select_set('label', 'Default Customer Type')
                ->input_select_set('icon', 'fa fa-info')
                ->input_select_set('min_length', '0')
                ->input_select_set('id', $id_prefix . '_customer_type_default')
                ->input_select_set('data_add', $bool_arr)
                ->input_select_set('value', array())
                ->input_select_set('hide_all', true)
                ->input_select_set('allow_empty', true)
        ...
        $form->input_select_add()
                ...
                ->input_select_set('icon', 'fa fa-info')
                ->input_select_set('min_length', '0')
                ->input_select_set('id', $id_prefix . '_customer_type_status')
        ...
        if ($is_modal) {
            $param['detail_tab'] = '#modal_customer_type .modal-body';
            $param['view_url'] = '';
            $param['window_scroll'] = '#modal_customer_type';
        }

        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'] . 'customer_type/customer_type_basic_function_js', $param, TRUE);
        $app->js_set($js);
    ...
    public static function customer_type_status_log_render($app, $form, $data, $path) {
        //<editor-fold defaultstate="collapsed">
        $config = array(
            'module_name' => 'customer_type',
