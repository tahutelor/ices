...
class Sales_Receipt_Renderer {
    ...
    public static function modal_sales_receipt_render($app, $modal) {
        //<editor-fold defaultstate="collapsed">
        $modal->header_set(array('title' => Lang::get('Sales Receipt'), 'icon' => APP_ICON::sales_receipt()));
        $modal->width_set('75%');
    ...
    public static function sales_receipt_components_render($app, $form, $is_modal) {
        ...
        $form->datetimepicker_add()->datetimepicker_set('label', Lang::get(array('Sales Receipt','Date')))
        ...
        $form->input_add()->input_set('label', Lang::get('Supplier Bank Account'))
                ->input_set('id', $id_prefix . '_customer_bank_account')
        ...
        if ($is_modal) {
            $param['detail_tab'] = '#modal_sales_receipt .modal-body';
            $param['view_url'] = '';
            $param['window_scroll'] = '#modal_sales_receipt';
        }
        
        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'] . 'sales_receipt/sales_receipt_basic_function_js', $param, TRUE);
        $app->js_set($js);
