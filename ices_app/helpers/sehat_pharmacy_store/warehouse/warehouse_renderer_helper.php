...
class Warehouse_Renderer {
    ...
    public static function modal_warehouse_render($app, $modal) {
        //<editor-fold defaultstate="collapsed">
        $modal->header_set(array('title' => Lang::get('Warehouse Controller'), 'icon' => 'fa fa-cogs'));
        $modal->width_set('95%');
        $components = self::warehouse_components_render($app, $modal, true);
        //</editor-fold>
    }

    public static function warehouse_render($app, $form, $data, $path, $method) {
        //<editor-fold defaultstate="collapsed">
        $path = Warehouse_Engine::path_get();

        $id_prefix = Warehouse_Engine::$prefix_id;

        $id = $data['id'];
        $components = self::warehouse_components_render($app, $form, false);
    ...
    public static function warehouse_components_render($app, $form, $is_modal) {
        //<editor-fold defaultstate="collapsed">
        $path = Warehouse_Engine::path_get();
        $components = array();
        $db = new DB();
        ...
        $form->input_select_add()
                ...
                ->input_select_set('id', $id_prefix . '_warehouse_status')
        ...
        if ($is_modal) {
            $param['detail_tab'] = '#modal_warehouse .modal-body';
            $param['view_url'] = '';
            $param['window_scroll'] = '#modal_warehouse';
        }

        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'] . 'warehouse/warehouse_address_js', $param, TRUE);
        $app->js_set($js);

        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'] . 'warehouse/warehouse_phone_number_js', $param, TRUE);
        $app->js_set($js);

        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'] . 'warehouse/warehouse_basic_function_js', $param, TRUE);
        $app->js_set($js);
    ...
    public static function warehouse_status_log_render($app, $form, $data, $path) {
        //<editor-fold defaultstate="collapsed">
        $config = array(
