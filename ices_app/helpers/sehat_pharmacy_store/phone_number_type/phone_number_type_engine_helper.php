...
class Phone_Number_Type_Engine {
            ...
            case self::$prefix_method . '_inactive':
                ...
                if ($success === 1) {
                    ...
                    $q = '
                        ...
                        where sc.status > 0
                            and (
                                sc.name = ' . $db->escape($name) 
                                .' or sc.code = ' . $db->escape($code) 
                            . ')
    ...
    public function phone_number_type_add($db, $final_data, $id = '') {
        ...
        if ($success == 1) {
            $phone_number_type_id = $db->fast_get('phone_number_type'
                            , array('code' => $fphone_number_type['code'],
                            'status' => 1
                            )
                    )[0]['id'];
