...
class Contact_Category_Renderer {
    ...
    public static function modal_contact_category_render($app, $modal) {
        //<editor-fold defaultstate="collapsed">
        $modal->header_set(array('title' => Lang::get('Contact Category Controller'), 'icon' => 'fa fa-cogs'));
        $modal->width_set('95%');
        $components = self::contact_category_components_render($app, $modal, true);
        //</editor-fold>
    }

    public static function contact_category_render($app, $form, $data, $path, $method) {
        //<editor-fold defaultstate="collapsed">
        $path = Contact_Category_Engine::path_get();

        $id_prefix = Contact_Category_Engine::$prefix_id;

        $id = $data['id'];
        $components = self::contact_category_components_render($app, $form, false);
    ...
    public static function contact_category_components_render($app, $form, $is_modal) {
        $path = Contact_Category_Engine::path_get();
        $components = array();
        $db = new DB();
        ...
        $components[$id_prefix . '_status'] = $form->input_select_add()
                ...
                ->input_select_set('id', $id_prefix . '_contact_category_status')
        ...
        if ($is_modal) {
            $param['detail_tab'] = '#modal_contact_category .modal-body';
            $param['view_url'] = '';
            $param['window_scroll'] = '#modal_contact_category';
        }

        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'] . 'contact_category/contact_category_basic_function_js', $param, TRUE);
        $app->js_set($js);

        return $components;
    }

    public static function contact_category_status_log_render($app, $form, $data, $path) {
        $config = array(
            'module_name' => 'contact_category',
