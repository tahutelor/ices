...
class Company_Renderer {
    ...
    public static function modal_company_render($app, $modal) {
        //<editor-fold defaultstate="collapsed">
        $modal->header_set(array('title' => Lang::get('Company Controller'), 'icon' => 'fa fa-cogs'));
        $modal->width_set('95%');
        $components = self::company_components_render($app, $modal, true);
        //</editor-fold>
    }

    public static function company_render($app, $form, $data, $path, $method) {
        //<editor-fold defaultstate="collapsed">
        $path = Company_Engine::path_get();

        $id_prefix = Company_Engine::$prefix_id;

        $id = $data['id'];
        $components = self::company_components_render($app, $form, false);
    ...
    public static function company_components_render($app, $form, $is_modal) {
        $path = Company_Engine::path_get();
        $components = array();
        $db = new DB();
        ...
        $components[$id_prefix . '_status'] = $form->input_select_add()
                ...
                ->input_select_set('id', $id_prefix . '_company_status')
        ...
        if ($is_modal) {
            $param['detail_tab'] = '#modal_company .modal-body';
            $param['view_url'] = '';
            $param['window_scroll'] = '#modal_company';
        }

        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'] . 'company/company_basic_function_js', $param, TRUE);
        $app->js_set($js);

        return $components;
    }

    public static function company_status_log_render($app, $form, $data, $path) {
        $config = array(
            'module_name' => 'company',
