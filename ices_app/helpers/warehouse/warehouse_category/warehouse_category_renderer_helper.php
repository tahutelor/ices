...
class Warehouse_Category_Renderer {
    ...
    public static function modal_warehouse_category_render($app, $modal) {
        //<editor-fold defaultstate="collapsed">
        $modal->header_set(array('title' => Lang::get('Warehouse Category Controller'), 'icon' => 'fa fa-cogs'));
        $modal->width_set('95%');
        $components = self::warehouse_category_components_render($app, $modal, true);
        //</editor-fold>
    }

    public static function warehouse_category_render($app, $form, $data, $path, $method) {
        //<editor-fold defaultstate="collapsed">
        $path = Warehouse_Category_Engine::path_get();

        $id_prefix = Warehouse_Category_Engine::$prefix_id;

        $id = $data['id'];
        $components = self::warehouse_category_components_render($app, $form, false);
    ...
    public static function warehouse_category_components_render($app, $form, $is_modal) {
        $path = Warehouse_Category_Engine::path_get();
        $components = array();
        $db = new DB();
...
//        ;
        ...
        $components[$id_prefix . '_status'] = $form->input_select_add()
                ...
                ->input_select_set('id', $id_prefix . '_warehouse_category_status')
        ...
        if ($is_modal) {
            $param['detail_tab'] = '#modal_warehouse_category .modal-body';
            $param['view_url'] = '';
            $param['window_scroll'] = '#modal_warehouse_category';
        }

        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'] . 'warehouse_category/warehouse_category_basic_function_js', $param, TRUE);
        $app->js_set($js);

        return $components;
    }

    public static function warehouse_category_status_log_render($app, $form, $data, $path) {
        $config = array(
            'module_name' => 'warehouse_category',
