...
class Contact_Renderer {
    ...
    public static function modal_contact_render($app, $modal) {
        //<editor-fold defaultstate="collapsed">
        $modal->header_set(array('title' => Lang::get('Contact Controller'), 'icon' => 'fa fa-cogs'));
        $modal->width_set('95%');
        $components = self::contact_components_render($app, $modal, true);
        //</editor-fold>
    }

    public static function contact_render($app, $form, $data, $path, $method) {
        //<editor-fold defaultstate="collapsed">
        $path = Contact_Engine::path_get();

        $id_prefix = Contact_Engine::$prefix_id;

        $id = $data['id'];
        $components = self::contact_components_render($app, $form, false);
    ...
    public static function contact_components_render($app, $form, $is_modal) {
        $path = Contact_Engine::path_get();
        $components = array();
        $db = new DB();
        ...
        $form->table_input_add()->table_input_set('id', $id_prefix . '_tbl_contact_category')
                ->main_div_set('class', 'form-group hide_all')
                ->label_set('value', '')
                ->table_input_set('columns', array(
                    'col_name' => 'contact_category'
                    , 'col_id_exists' => true
                    , 'th' => array('val' => 'Contact Category', 'visible' => true)
        ...
        $form->datetimepicker_add()->datetimepicker_set('label', Lang::get(array('Birthdate', 'Date')))
        ...
        $components[$id_prefix . '_status'] = $form->input_select_add()
                ...
                ->input_select_set('id', $id_prefix . '_contact_status')
        ...
        if ($is_modal) {
            $param['detail_tab'] = '#modal_contact .modal-body';
            $param['view_url'] = '';
            $param['window_scroll'] = '#modal_contact';
        }
        ...
        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'] . 'contact/contact_phone_number_js', $param, TRUE);
        $app->js_set($js);

        $js = get_instance()->load->view(ICES_Engine::$app['app_base_dir'] . 'contact/contact_basic_function_js', $param, TRUE);
        $app->js_set($js);
        
        return $components;
    }

    public static function contact_status_log_render($app, $form, $data, $path) {
        $config = array(
            'module_name' => 'contact',
